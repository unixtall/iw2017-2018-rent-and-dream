...
public class UserEditor extends FormLayout {
	...
	ComboBox <UserRoleType> role = new ComboBox <UserRoleType>("role");
	...
	public UserEditor(UserService service) {
		...
		// Add some items
		role.setItems(EnumSet.allOf(UserRoleType.class));
		role.setSelectedItem(UserRoleType.USER);
		...
		// Solo borra el admin
		role.setVisible(SecurityUtils.hasRole(UserRoleType.ADMIN));
		registerDate.setVisible(SecurityUtils.hasRole(UserRoleType.ADMIN));
